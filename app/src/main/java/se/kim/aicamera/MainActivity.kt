...
class MainActivity : AppCompatActivity() {
 ...
 override fun onCreate(b:Bundle?){super.onCreate(b);setContentView(R.layout.activity_main);status=findViewById(R.id.status);previewView=findViewById(R.id.preview)
  findViewById<Button>(R.id.shutter).setOnClickListener{v->if(timerSeconds>0){status.text="Tar foto om "+timerSeconds+" s";v.postDelayed({takePhoto()},timerSeconds*1000L)}else takePhoto()}
  findViewById<Button>(R.id.flash).setOnClickListener{v->flashMode=when(flashMode){ImageCapture.FLASH_MODE_OFF->ImageCapture.FLASH_MODE_AUTO;ImageCapture.FLASH_MODE_AUTO->ImageCapture.FLASH_MODE_ON;else->ImageCapture.FLASH_MODE_OFF};capture?.flashMode=flashMode;(v as Button).text=when(flashMode){ImageCapture.FLASH_MODE_AUTO->"Blixt auto";ImageCapture.FLASH_MODE_ON->"Blixt på";else->"Blixt av"}}
  findViewById<Button>(R.id.timer).setOnClickListener{v->timerSeconds=when(timerSeconds){0->3;3->10;else->0};(v as Button).text=if(timerSeconds==0)"Timer av" else "Timer "+timerSeconds+"s"}
  findViewById<Button>(R.id.gridBtn).setOnClickListener{val g=findViewById<View>(R.id.grid);g.visibility=if(g.visibility==View.VISIBLE)View.GONE else View.VISIBLE}
  findViewById<Button>(R.id.switchCamera).setOnClickListener{lens=if(lens==CameraSelector.LENS_FACING_BACK)CameraSelector.LENS_FACING_FRONT else CameraSelector.LENS_FACING_BACK;bindCamera()}
  findViewById<Button>(R.id.zoom05).setOnClickListener{setZoom(0.5f)};findViewById<Button>(R.id.zoom1).setOnClickListener{setZoom(1f)};findViewById<Button>(R.id.zoom2).setOnClickListener{setZoom(2f)}
  findViewById<Button>(R.id.modePhoto).setOnClickListener{selectMode("photo","Foto")};findViewById<Button>(R.id.modePortrait).setOnClickListener{selectMode("portrait","Porträtt • AI-bokeh")};findViewById<Button>(R.id.modeNight).setOnClickListener{selectMode("night","Natt • AI-optimering")}
